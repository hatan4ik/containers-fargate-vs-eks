...
| allow\_insecure\_http | Explicitly permit HTTP-only ingress when no certificate is available. | `bool` | `false` | no |
...
| application\_log\_retention\_days | Application log retention in days. | `number` | `30` | no |
| certificate\_arn | Validated ACM certificate ARN for ALB HTTPS. Null requires explicit HTTP-only opt-in. | `string` | `null` | no |
...
| flow\_log\_retention\_days | VPC flow-log retention in days. | `number` | `30` | no |
