...
| cluster\_log\_retention\_days | EKS control-plane log retention in days. | `number` | `30` | no |
| endpoint\_public\_access | Expose the Kubernetes API publicly. Requires explicit CIDRs. | `bool` | `false` | no |
| endpoint\_public\_access\_cidrs | Explicit CIDRs allowed to reach the EKS public endpoint. | `list(string)` | `[]` | no |
